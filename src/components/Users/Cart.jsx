...
const Cart = () => {
  ...
  const handleQuantityChange = (ticketId, action) => {
    const updatedCart = [...cart];
    const ticketIndex = updatedCart.findIndex((item) => item.id === ticketId);

    if (ticketIndex !== -1) {
      if (action === "increase") {
        updatedCart[ticketIndex].quantity += 1;
      } else if (
        action === "decrease" &&
        updatedCart[ticketIndex].quantity > 1
      ) {
  ...
  const handleRemoveItem = (ticketId) => {
    const updatedCart = cart.filter((item) => item.id !== ticketId);
    localStorage.setItem("cart", JSON.stringify(updatedCart));
    setCart(updatedCart);
  };
      ...
      {cart.length > 0 ? (
        cart.map((ticket) => (
          <div key={ticket.id} className="ticket-item">
            ...
            <button onClick={() => handleRemoveItem(ticket.id)}>Remove</button>
            <hr />
          </div>
        ))
      ) : (
